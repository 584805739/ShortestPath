...
bool ShortestPathUtil::construct_index_ext() {
    ...
    ofstream outfile;
    outfile.open("index_out.txt");
    ofstream testfile;
    testfile.open("outcome.txt");
    int num_vertices = _ptr_ugraph->get_vertices_num();
    std::vector<bool> vec_used_as_root(num_vertices, false);
...
/*
            ...
            for (size_t sz = 0; sz < buf_index_u.first.size(); ++sz) {
                int w = buf_index_u.first[sz];
                if (w < 0 || dst_v[w] == -1) {//w < 0 means p label
                    continue;
                }
                int td = buf_index_u.second[sz] + dst_v[w];
                if (td <= dst_v[uid]) {
            ...
            std::vector<int>::iterator iter_set = map_vertice2paths[uid].begin();
            for (; !prune_flag && iter_set != map_vertice2paths[uid].end(); ++iter_set) {
                int path_id = abs(*iter_set) - 1;
                expand_plabel(uid, vec_paths[path_id], &vec_expanded_label);
                for (size_t pos = 0; pos < vec_expanded_label.size(); ++ pos) {
                    std::pair<int, int>& pair_ref = vec_expanded_label.at(pos);
                    int w = pair_ref.first;
                    if (dst_v[w] == -1) {
                        continue;
                    }
                    int td = pair_ref.second + dst_v[w];
                    if (td <= dst_v[uid]) {
                        prune_flag = true;
                        break;
                    }
...
*/          if (prune_flag) {
        ...
        //get the most valuable longest path
            ...
            if((vec_longest_path.size()>3)&&(max_vertices_new >= vec_longest_path.size()*0.57))//
            {
    ...
    for (int i = 0; i < _num_paths; ++i) {
        ...
        for (size_t sz = 0; sz < size; ++sz) {
            ...
            testfile << _ptr_paths[i].ptr_vertices[sz] << ' ';
    ...
    ofstream min_out;
    min_out.open("min_out.txt");
    for (size_t i = 0; i < vec_degree.size(); ++i) {
        int vertice = vec_degree.at(i);

        size_t size = buf_index[vertice].first.size();
        labeltotal += size;
        min_out<<"vertice"<< vertice  <<"labeltotal:  "<<labeltotal<<"        ";
        _ptr_index[vertice].ptr_lv = (int*)calloc(size, sizeof(int));
        _ptr_index[vertice].ptr_ld = (int*)calloc(size, sizeof(int));
        if (_ptr_index[vertice].ptr_lv == NULL
                || _ptr_index[vertice].ptr_ld == NULL) {
        ...
        min_out<<"_ptr_index size:  "<< _ptr_index[vertice].size<<endl;
        outfile << endl;
        //free the memory of vector
        std::vector<int>().swap(buf_index[vertice].first);
        std::vector<int>().swap(buf_index[vertice].second);
...
unsigned long long ShortestPathUtil::compute_index_mem() const {
...
unsigned long long ShortestPathUtil::compute_index_mem_ext() const {
    unsigned long long mem_sum = 0;
    if (_ptr_index == NULL) {
        return mem_sum;
    }
    ...
    std::cout<<"path_memory: "<< mem_sum<<std::endl;

    ofstream outfile;
    outfile.open("compute_mem_outcome.txt");
    int num_vertices = _ptr_ugraph->get_vertices_num();
    for (int i = 0; i < num_vertices; ++i) {
        if (_ptr_index[i].ptr_lv == NULL || _ptr_index[i].ptr_ld == NULL) {
            continue;
        }
        outfile<<_ptr_index[i].size<<endl;
