...
bool ShortestPathUtil::construct_index() {
    ...
    for (int v = 0; v < vec_degree.size(); ++v) {
        ...
        /*
        std::vector<int> dst_r(num_vertices, -1);
    ...
    for (unsigned int i = 0; i < vec_degree.size(); ++i) {
...
bool ShortestPathUtil::construct_index_ext() {
    ...
    std::map<int, std::vector<int> > map_expand_labels; // store vertice's expand labels
...
*/          if (prune_flag) {
	...
	while(count_times < vec_last_level_vertices.size())//vec_last_level_vertices.size()
	{
    ...
    //get the most valuable longest path
        ...
        if (prune_flag && vec_last_level_vertices.empty()) {
            ...
            for (size_t sz = 0; sz < vec_last_level_vertices.size(); ++sz) {
                ...
                while (vertice_id != -1) {
                	/*
                   if (set_vertice_in_paths.find(vertice_id) == set_vertice_in_paths.end()) {
                       ++cur_vertices_new;
                   }*/
        ...
        if( max_vertices_new < vec_longest_path.size()*3/4 )
        {
            break;
        }
        else
