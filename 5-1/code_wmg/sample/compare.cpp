...
int main(int argc, char** argv) {
    ...
    for ( ; i <= num_cases; ++i) {
        ...
        query_time1 -= get_current_time_in_mseconds();
        int dist_benchmark = pll.QueryDistance(w, v);
        query_time1 += get_current_time_in_mseconds();
        query_time2 -= get_current_time_in_mseconds();
        int dist_ours = sp_util.query_distance(w, v);
        query_time2 += get_current_time_in_mseconds();
    ...
    pll.PrintStatistics();
    std::cout << "=============================" << std::endl;
    std::cout << "Graph Loading+Indexing Time(msec):" << time_index << std::endl;
    std::cout << "Index Structure Mem(bytes):" << mem_index << std::endl;
    std::cout << "Average Query Time(msec):" << query_time1 << "(benchmark):"
