...
void UndirectedGraph::construct_graph(const std::vector<std::pair<int, int> >& vec_pairs) {
    ...
    // for (int i = 0; i < _num_vertices; ++i) {
    //     vec_deg_pair.clear();
    //     for(size_t j = 0;j < _pp_buf[i].size(); ++j)
    //     {
    //         adj_vertice = _pp_buf[i].at(j);
    //         degree = _pp_buf[adj_vertice].size();
    //         vec_deg_pair.push_back(std::pair<int, int>(degree, adj_vertice));
    //     }  
    //     //sort the vertices in descreasing order of degree
    ...
    //     _pp_buf[i].clear();
    //     for(size_t j = 0;j < vec_deg_pair.size(); ++j)
    //     {
    //         _pp_buf[i].push_back(vec_deg_pair[j].second);
    //     }
